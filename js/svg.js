...
function SVG(template) {
    ...
    let stencilNodes=node.querySelectorAll("[inkscape\\:label=\"stencils\"]")[0].childNodes;
    for (let i=0;i<stencilNodes.length;i++)
        stencils.push(stencilNodes[i]);
    ...
    this.finalize=function() {
        stencils.forEach(stencil=>{
            if (stencil.parentNode) stencil.parentNode.removeChild(stencil);
        });
    ...
    this.downloadRasterPDF = async(filename) => {
        ...
        let
            svgstring=this.getSVG(),
            image=document.createElement("img");
        
        document.body.appendChild(image);
        image.width=SHEETWIDTH*SCALE;
        image.height=SHEETHEIGHT*SCALE;
        ...
        image.onload = ()=>{
            setTimeout(()=>{
                ...
                doc.addImage(imgData, 'PNG', 0, 0, SHEETWIDTH, SHEETHEIGHT);
                doc.save(filename);
